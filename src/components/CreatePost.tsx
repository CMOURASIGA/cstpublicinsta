...
export default function CreatePost({ onPostCreated, currentUser }: CreatePostProps) {
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      await handleFileUploadSimulation(file);
    }
  };
  ...
  const handleFileUploadSimulation = async (file: File) => {
    ...
    // Read to base64 if it is an image to show actual local client previews!
    let base64Result = '';
    ...
    setTimeout(async () => {
      try {
        const res = await fetch('/api/simulate-drive-upload', {
          method: 'POST',
              ...
              {uploading ? (
                ...
                <div className="space-y-2 py-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1">Carregado no Drive. ID: <code className="bg-slate-200 px-1 py-0.5 rounded text-amber-800">{fileId}</code></p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
