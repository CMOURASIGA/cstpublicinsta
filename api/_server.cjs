...
var INSTAGRAM_CONTAINER_WAIT_TIMEOUT_MS = Number(process.env.INSTAGRAM_CONTAINER_WAIT_TIMEOUT_MS || 24e4);
...
function renderOauthPopupResultPage(title, message, payload) {
...
<html lang="pt-BR">
  ...
  <body>
    <main>
      ...
      <p class="muted" id="popup-status">Tentando devolver o resultado para a aplica\xE7\xE3o que abriu esta janela.</p>
      <p class="muted">Se esta janela n\xE3o fechar automaticamente, finalize manualmente e volte para a aplica\xE7\xE3o.</p>
    </main>
    <script>
      (function () {
        ...
        if (canAutoClose) {
          if (status) status.textContent = 'Conex\xE3o conclu\xEDda. Fechando a janela...';
          window.close();
          return;
        }
        if (status) {
          status.textContent = 'Conex\xE3o conclu\xEDda, mas a janela foi mantida aberta porque o retorno aconteceu em outra origem.';
        }
      })();
    </script>
  </body>
...
async function ensureInstagramAccessToken(context) {
  ...
  const refreshThreshold = Date.now() + 7 * 24 * 60 * 60 * 1e3;
...
async function instagramGraphRequest(context, resource, init) {
  ...
  const nextInit = init ? {
    ...init,
    body: shouldReplaceToken && init.body instanceof URLSearchParams ? new URLSearchParams(
      init.body.toString().replaceAll(context.instagramAccessToken, usableToken)
    ) : shouldReplaceToken && typeof init.body === "string" ? init.body.replaceAll(context.instagramAccessToken, usableToken) : init.body
  } : init;
...
async function exchangeInstagramShortLivedForLongLivedToken(shortLivedToken) {
  const config = getRuntimeConfig();
...
async function refreshInstagramLongLivedAccessToken(currentToken) {
...
function resolveExpiresAtFromSeconds(expiresIn) {
...
app.get("/api/integrations/google-drive/callback", async (req, res) => {
  try {
    ...
    return res.status(200).send(
      renderOauthPopupResultPage("Google OAuth conclu\xC3\xADdo", payload.message, {
  ...
  try {
    ...
    if (resolvedState?.cliente_id) {
      ...
      await addLog("Instagram API", "error", "Falha no callback do Instagram Login.", {
        ...
        error: maskError(error)
      }, resolvedState.cliente_id).catch(() => void 0);
    }
    return res.status(error instanceof HttpError ? error.status : 400).send(
